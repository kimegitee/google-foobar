...
def getnext(n: str, b: int) -> str:
    '''Get next number in sequence'''
    k = len(n)
    n_dsc = int(''.join(sorted(n, reverse=True)), base=b)
    n_asc = int(''.join(sorted(n)), base=b)
    n = int2base(n_dsc - n_asc, b)
    return '{:0>{k}}'.format(n, k=k)

def int2base(n: int, b: int) -> str:
    '''Convert int from base 10 to base b'''
    if n == 0:
        return '0'
    digits = ''
    while n:
